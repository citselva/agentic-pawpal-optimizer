...
@dataclass
class ApiCallRecord:
    """Metrics for a single Anthropic API call.

    Attributes
    ----------
    call_index:
        1-based position of this call within the current orchestrator session.
    method:
        Human-readable label — ``"parse_nl_task"`` or ``"resolve_step_N"``.
    model:
        The model ID used (``NL_MODEL`` or ``RESOLUTION_MODEL``).
    latency_ms:
        Wall-clock duration of the ``client.messages.create()`` round-trip,
        in milliseconds.
    input_tokens:
        Tokens billed at the full input rate (non-cached tokens only).
    output_tokens:
        Tokens generated by the model.
    cache_creation_tokens:
        Tokens written to the prompt cache on this call
        (``usage.cache_creation_input_tokens``).  Non-zero only on the first
        call in a cache window, or when the system prompt changes.
    cache_read_tokens:
        Tokens served from the prompt cache
        (``usage.cache_read_input_tokens``).  Non-zero on subsequent calls
        while the cache entry is warm (within ~5 minutes).
    """
    ...
    def total_tokens(self) -> int:
        """All tokens consumed by this call (input + output + cache reads)."""
        return (
            self.input_tokens
            + self.output_tokens
    ...
    def cache_saved(self) -> bool:
        """``True`` when this call benefited from a warm prompt-cache entry."""
...
@dataclass
class RunMetrics:
    """Aggregated token usage and latency for one full orchestrator session.

    Updated after every ``_call_api()`` invocation.  Surfaced to the
    Streamlit UI via :attr:`PawPalOrchestrator.run_metrics` so owners can
    see the efficiency of the prompt-caching strategy at a glance.

    The key explainability numbers are:

    * ``total_cache_read_tokens`` — how many input tokens were served for
      free (or at a fraction of the cost) from the cache.
    * ``effective_token_reduction_pct`` — the percentage of potential input
      tokens that hit the cache, directly proving that the static system
      prompt head is not re-tokenised on every call.
    * ``cache_hit_rate`` — fraction of API calls that had at least one cache
      read token, showing the caching strategy fired consistently.
    """
    ...
    def add(self, record: ApiCallRecord) -> None:
        """Append one completed call record."""
        self.calls.append(record)

    def reset(self) -> None:
        """Clear all records (called alongside :meth:`PawPalOrchestrator.clear_trace`)."""
        self.calls.clear()
    ...
    def total_input_tokens(self) -> int:
        """Tokens billed at the full (non-cached) input rate."""
    ...
    def total_cache_creation_tokens(self) -> int:
        """Tokens written to the prompt cache across all calls."""
        return sum(c.cache_creation_tokens for c in self.calls)

    @property
    def total_cache_read_tokens(self) -> int:
        """Tokens served from the prompt cache across all calls."""
        return sum(c.cache_read_tokens for c in self.calls)

    @property
    def cache_hit_rate(self) -> float:
        """Fraction of calls (0.0–1.0) that read at least one token from cache."""
        if not self.calls:
            return 0.0
        return sum(1 for c in self.calls if c.cache_saved) / len(self.calls)

    @property
    def effective_token_reduction_pct(self) -> float:
        """Percentage of potential input tokens delivered from cache.

        Computed as ``cache_read / (input + cache_creation + cache_read)``.
        A value of 90 % means 90 % of what would have been billed at full
        input price was instead served from the cache — a direct proof that
        ...
        if potential == 0:
            return 0.0
        return self.total_cache_read_tokens / potential * 100.0
    ...
    def as_dict(self) -> dict[str, Any]:
        """Full metrics dict for JSON logging or Streamlit session state."""
        return {
            ...
            "effective_token_reduction_pct": round(
                self.effective_token_reduction_pct, 1
            ),
            "calls": [c.as_dict() for c in self.calls],
        }

    def format_summary(self) -> str:
        """Markdown table suitable for a Streamlit ``st.markdown()`` block.

        Renders the cache-efficiency story in a compact, shareable format::

            ### Token & Latency Report
            | Metric                  | Value     |
            |-------------------------|-----------|
            | API calls               | 3         |
            | Total latency           | 2 340 ms  |
            | Input tokens (billed)   | 312       |
            | Output tokens           | 487       |
            | Cache write tokens      | 2 941     |
            | Cache read tokens       | 5 882     |
            | Cache hit rate          | 67% of calls |
            | Token reduction (cache) | 63.2%     |
        """
        rows = [
            ("API calls", str(self.total_calls)),
            ("Total latency", f"{self.total_latency_ms:,.0f} ms"),
            ("Input tokens (billed)", f"{self.total_input_tokens:,}"),
            ("Output tokens", f"{self.total_output_tokens:,}"),
            ("Cache write tokens", f"{self.total_cache_creation_tokens:,}"),
            ("Cache read tokens", f"{self.total_cache_read_tokens:,}"),
            (
                "Cache hit rate",
                f"{self.cache_hit_rate * 100:.0f}% of calls",
            ),
            (
                "Token reduction (cache)",
                f"{self.effective_token_reduction_pct:.1f}%",
            ),
        ]
        header = "### Token & Latency Report\n| Metric | Value |\n|--------|-------|"
...
class PawPalOrchestrator:
    ...
    run_metrics:
        Accumulated :class:`RunMetrics` for the current session — token
        counts, latency, and cache-efficiency figures.  Reset alongside
        ``agent_trace`` by :meth:`clear_trace`.
    """
    ...
    def clear_trace(self) -> None:
        """Reset the trace list and token-usage metrics between sessions."""
        self.agent_trace = []
        self.run_metrics.reset()

    # ------------------------------------------------------------------
    # API call wrapper — timing + usage capture
    # ------------------------------------------------------------------

    def _call_api(self, method_label: str, **kwargs: Any) -> Any:
        """Thin wrapper around ``client.messages.create()`` that records usage.

        All keyword arguments are forwarded verbatim to the SDK.  After the
        response returns, the ``usage`` object on the response is read and a
        new :class:`ApiCallRecord` is appended to :attr:`run_metrics`.

        Parameters
        ----------
        method_label:
            A short, human-readable tag stored in the record — e.g.
            ``"parse_nl_task"`` or ``"resolve_step_2"``.
        **kwargs:
            Any keyword arguments accepted by ``anthropic.messages.create()``,
            such as ``model``, ``max_tokens``, ``system``, ``tools``, and
            ``messages``.

        Returns
        -------
        anthropic.types.Message
            The raw SDK response, unchanged.
        """
        ...
        record = ApiCallRecord(
            call_index=self.run_metrics.total_calls + 1,
            method=method_label,
        ...
        self.run_metrics.add(record)

        logger.debug(
            "_call_api [#%d %s] %s — %.0f ms | "
